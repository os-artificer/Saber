...
const supervisorEnv = "SABER_TRANSFER_SUPERVISOR"

func pidFilePath() string {
	return filepath.Join("pids", "transfer.pid")
}
...
// args from os.Executable() and os.Args[1:]). Called when SABER_TRANSFER_SUPERVISOR=1.
...
func runStop(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return fmt.Errorf("transfer not running")
		}
...
func runRestart(cmd *cobra.Command, args []string) error {
	if err := runStop(cmd, args); err != nil && err.Error() != "transfer not running" {
		return err
	}
