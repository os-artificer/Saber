...
const supervisorEnv = "SABER_ADMIN_SUPERVISOR"

func pidFilePath() string {
	return filepath.Join("pids", "admin.pid")
}
...
// RunSupervisor runs the daemon loop in the current process. Called when SABER_ADMIN_SUPERVISOR=1.
...
func runStop(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return fmt.Errorf("admin not running")
		}
...
func runRestart(cmd *cobra.Command, args []string) error {
	if err := runStop(cmd, args); err != nil && err.Error() != "admin not running" {
		return err
	}
