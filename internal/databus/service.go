...
// databusUnmarshalOpt composes default viper hooks with string->Endpoint so
// service.listenAddress (string) unmarshals into sbnet.Endpoint.
var databusUnmarshalOpt = viper.DecodeHook(mapstructure.ComposeDecodeHookFunc(
...
func CreateService(ctx context.Context, serviceID string) (*Service, error) {
	snk, err := sink.NewSinkFromConfig(config.Cfg.Sink)
	if err != nil {
		return nil, err
	}
...
// InitAPM creates the APM service from config.Cfg.APM and sets s.apm. Business metrics are in internal/databus/apm/metrics.go.
...
func (s *Service) ReloadConfig() error {
	...
	if err := viper.Unmarshal(&config.Cfg, databusUnmarshalOpt); err != nil {
		return err
	}
...
// RegisterSelf registers the databus service with the discovery service (etcd).
func (s *Service) RegisterSelf() error {
	...
	opts := []discovery.Option{
		...
		discovery.OptionServiceName("databus"),
	...
	listenAddr := getDatabusListenAddr()
	...
	logger.Infof("databus registered to discovery, serviceID=%s, listenAddress=%s", serviceID, listenAddr)
	return nil
}
...
// loadDatabusConfig reads config from ConfigFilePath into config.Cfg.
func loadDatabusConfig() {
	if ConfigFilePath == "" {
		return
	}
	...
	logger.Infof("Loaded databus config: %+v", config.Cfg)
}
