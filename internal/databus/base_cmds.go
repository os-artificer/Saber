...
const supervisorEnv = "SABER_DATABUS_SUPERVISOR"

func pidFilePath() string {
	return filepath.Join("pids", "databus.pid")
}
...
// args from os.Executable() and os.Args[1:]). Called when SABER_DATABUS_SUPERVISOR=1.
...
func runStop(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return fmt.Errorf("databus not running")
		}
...
func runRestart(cmd *cobra.Command, args []string) error {
	if err := runStop(cmd, args); err != nil && err.Error() != "databus not running" {
		return err
	}
